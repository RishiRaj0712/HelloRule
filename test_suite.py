...
==========================================================
Tests answer quality + edge cases across Constitution, BNS, and BNSS.

Usage:
    python3 test_suite.py                    # run all tests
    python3 test_suite.py --category fr      # only Fundamental Rights
    python3 test_suite.py --category bnss    # only BNSS (Criminal Procedure)
...
Categories:
    ...
    bns     Criminal Law (BNS) — offences & punishments
    bnss    Criminal Procedure (BNSS) — arrest, bail, FIR, trial, appeals
"""
...
TEST_CASES = [
    ...
    },
    {
        ...
        "expect_cite": ["173"],
        "expect_hit": ["BNSS", "cognizable", "police"],
        "expect_miss": [],
        "note": "FIR procedure is now in scope via BNSS Section 173",
    },
    {
        "id": "EDGE-03",
        "category": "edge",
    ...
    # ══════════════════════════════════════════════
    # CATEGORY 6: Criminal Procedure (BNSS)
    # ══════════════════════════════════════════════
    {
        "id": "BNSS-01",
        "category": "bnss",
        "query": "What is the procedure for bail in a non-bailable offence?",
        "expect_cite": ["480"],
        "expect_hit": ["bail", "non-bailable"],
        "expect_miss": [],
        "note": "BNSS Section 480 — bail in non-bailable offences (old CrPC 437)",
    },
    {
        "id": "BNSS-02",
        "category": "bnss",
        "query": "Can police arrest someone without a warrant under BNSS?",
        "expect_cite": ["35"],
        "expect_hit": ["arrest", "warrant", "cognizable"],
        "expect_miss": [],
        "note": "BNSS Section 35 — arrest without warrant (old CrPC 41)",
    },
    {
        "id": "BNSS-03",
        "category": "bnss",
        "query": "How to file a Zero FIR under BNSS?",
        "expect_cite": ["173"],
        "expect_hit": ["FIR", "cognizable", "police station"],
        "expect_miss": [],
        "note": "BNSS Section 173 — Zero FIR (new provision replacing CrPC 154)",
    },
    {
        "id": "BNSS-04",
        "category": "bnss",
        ...
        "expect_cite": ["482"],
        "expect_hit": ["bail", "apprehending arrest", "non-bailable"],
        "expect_miss": [],
        "note": "BNSS Section 482 — anticipatory bail (old CrPC 438)",
    },
    {
        "id": "BNSS-05",
        "category": "bnss",
        "query": "What is CrPC Section 41 in the new criminal law?",
        "expect_cite": ["35"],
        "expect_hit": ["BNSS", "arrest"],
        "expect_miss": [],
        "note": "CrPC-to-BNSS translation — CrPC 41 = BNSS 35",
    },
    {
        "id": "BNSS-06",
        "category": "bnss",
        "query": "Can a trial be conducted through video conferencing?",
        "expect_cite": ["530"],
        "expect_hit": ["electronic", "trial"],
        "expect_miss": [],
        "note": "BNSS Section 530 — trials in electronic mode (new provision)",
    },
    {
        "id": "BNSS-07",
        "category": "bnss",
        "query": "What is the time limit for police to complete investigation?",
        "expect_cite": ["187"],
        "expect_hit": ["investigation", "days"],
        "expect_miss": [],
        "note": "BNSS Section 187 — investigation completion timeline",
    },
    {
        "id": "BNSS-08",
        "category": "bnss",
        "query": "What is plea bargaining and when can it be applied?",
        "expect_cite": ["289", "290"],
        "expect_hit": ["plea bargaining"],
        "expect_miss": [],
        "note": "BNSS Chapter XXIII — plea bargaining provisions (Sections 289-300)",
    },
    {
        "id": "BNSS-09",
        "category": "bnss",
        ...
        "expect_cite": ["472"],
        "expect_hit": ["mercy", "death", "sentence"],
        "expect_miss": [],
        "note": "BNSS Section 472 — mercy petition timeline (30 days Governor, 60 days President)",
    },
    {
        "id": "BNSS-10",
        "category": "bnss",
        "query": "Is forensic investigation mandatory for serious crimes under BNSS?",
        "expect_cite": ["176"],
        "expect_hit": ["forensic", "seven years"],
        "expect_miss": [],
        "note": "BNSS Section 176 — mandatory forensic for offences with 7+ years imprisonment",
    },
]
...
CATEGORY_NAMES = {
    ...
    "bnss": "Criminal Procedure (BNSS)",
}

def main():
    parser = argparse.ArgumentParser(description="LawBook India — Full Test Suite")
    parser.add_argument("--category", choices=["fr", "emer", "sched", "edge", "bns", "bnss"], default=None)
    ...
    categories = [args.category] if args.category else ["fr", "emer", "sched", "edge", "bns", "bnss"]
